...
namespace Concrete\Flysystem\Util;
...
class MimeType
{
    ...
    public static function detectMimeType($path = "", $content = "", $default = null)
    {
        if (!empty($content) && $mime_type = static::detectByContent($content)) {
            if ($mime_type === 'text/plain' && $extension = pathinfo($path, PATHINFO_EXTENSION)) {
                if ($type = static::detectByFileExtension($extension)) {
                    return $type;
                }
            }
            return $mime_type;
        ...
        if (!empty($path) && $mime_type = static::detectByPath($path)) {
            return $mime_type;
        }
    ...
    protected static function detectByPath($path)
    {
